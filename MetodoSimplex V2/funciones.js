function convertir_desigualdades_en_igualdades(d) {
  ...
  for (let i = 0; i < d.length; i++) {
    ...
    if (desigualdad.includes("<=")) {
      ...
      igualdades.push(
        desigualdad.replace(">=", "-s(" + (i + 1) + ")+m(" + (i + 1) + ")=")
      );
      variables.push("-s(" + (i + 1) + ")");
...
function agregar_variables_func_obj(func_objetivo, variables) {
  ...
  let nueva_func_objetivo = func_objetivo.replace(
    /(?<!\d)(x\(\d+\))(?! *\*)/g,
    "1$1"
  );
  nueva_func_objetivo = nueva_func_objetivo.replace(
    /(\-?\d*)x\((\d+)\)/g,
    (match, p1, p2) => (p1 ? p1 : "1") + "x(" + p2 + ")"
  );

  const variables_x = variables
    .filter((varName) => varName.includes("x("))
    .sort((a, b) => {
      const numA = parseInt(a.match(/\d+/)[0]);
      const numB = parseInt(b.match(/\d+/)[0]);
      return numA - numB;
    });
  const variables_s = variables
    .filter((varName) => varName.includes("s("))
    .sort((a, b) => {
      const numA = parseInt(a.match(/\d+/)[0]);
      const numB = parseInt(b.match(/\d+/)[0]);
      return numA - numB;
    });
  const variables_m = variables
    .filter((varName) => varName.includes("m("))
    .sort((a, b) => {
      const numA = parseInt(a.match(/\d+/)[0]);
      const numB = parseInt(b.match(/\d+/)[0]);
      return numA - numB;
...
function crearVectorX(func_objetivo) {
  ...
  const variablesOrdenadas = variables.sort(
    (a, b) => posiciones[a] - posiciones[b]
  );
  const vectorX = variablesOrdenadas.map(
    (variable) => variable.match(/[a-zA-Z]+\(\d+\)/)[0]
  );
  return vectorX;
}
...
function crearMatrizA(restricciones, variables) {
    ...
    for (var i = 0; i < restriccionesSinB.length; i++) {
      ...
      for (var j = 0; j < variables.length; j++) {
        ...
        if (!restriccion.includes(variable)) {
          ...
          restriccion.indexOf(variable) === restriccion.indexOf(variable + "1")
        ) {
          nueva_restriccion = nueva_restriccion.replace(
            variable,
            "1" + variable
          );
        }
  ...
  function crearRestriccionesOrdenadas(restriccionesExplicitas, variables) {
    ...
    for (var j = 0; j < restriccionesExplicitas.length; j++) {
      ...
      var terminos_restriccion = restriccion.match(
        /-?\d*\/?\d*[a-z]\(\d+\)|-[a-z]\(\d+\)/g
      );
      var terminos_ordenados = terminos_restriccion.sort(function (a, b) {
        ...
        return (
          variable_posiciones[variable_a] - variable_posiciones[variable_b]
        );
      });
  ...
  function formarMatriz(restriccionesCoeficientes, variables) {
    const matrizA = math.reshape(restriccionesCoeficientes, [
      restriccionesCoeficientes.length,
      variables.length,
    ]);
    return matrizA;
  }
  ...
  var restriccionesCompletas = crearRestriccionesCompletas(
    restriccionesSinB,
    variables
  );
  var restriccionesExplicitas = crearRestriccionesExplicitas(
    restriccionesCompletas,
    variables
  );
  var restriccionesOrdenadas = crearRestriccionesOrdenadas(
...
function solucionesAlternativas(vectorX, vectorXb, resultado3) {
  ...
  const elementosRestantes = vectorX.filter(
    (elemento) => !vectorXb.includes(elemento)
  );
  const posicionesRestantes = elementosRestantes.map((elemento) =>
    vectorX.indexOf(elemento)
  );
  if (posicionesRestantes.some((posicion) => resultado3array[posicion] === 0)) {
    ...
    var solucionesalternativas = document.getElementById(
...
function incompatible(vectorXb, resultado3) {
  ...
  if (posicionesM.length > 0) {
    ...
    posicionesM.forEach((posicion) => {
      console.log(
        `'m' se encuentra en la posición ${posicion} y su elemento es ${vectorXb[posicion]}.`
      );
    });
  }
}
